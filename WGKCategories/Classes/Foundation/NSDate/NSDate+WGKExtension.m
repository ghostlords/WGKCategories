...
- (NSUInteger)wgk_day {
    return [NSDate wgk_day:self];
}

- (NSUInteger)wgk_month {
    return [NSDate wgk_month:self];
}

- (NSUInteger)wgk_year {
    return [NSDate wgk_year:self];
}

- (NSUInteger)wgk_hour {
    return [NSDate wgk_hour:self];
}

- (NSUInteger)wgk_minute {
    return [NSDate wgk_minute:self];
}

- (NSUInteger)wgk_second {
    return [NSDate wgk_second:self];
}
...
+ (NSString *)wgk_formatYMD:(NSDate *)date {
    return [NSString stringWithFormat:@"%zd-%zd-%zd",[date wgk_year],[date wgk_month], [date wgk_day]];
}
...
//    NSDate *lastdate = [date wgk_lastdayOfMonth];
    ...
    for (i = 1;[[date wgk_dateAfterDay:-7 * i] wgk_year] == year; i++) {
...
+ (NSDate *)wgk_begindayOfMonth:(NSDate *)date {
    return [self wgk_dateAfterDate:date day:-[date wgk_day] + 1];
}
...
- (NSInteger)wgk_weekday {
    return [NSDate wgk_weekday:self];
}
...
- (NSDate *)wgk_dateByAddingDays:(NSUInteger)days {
...
+ (NSString *)wgk_stringWithDate:(NSDate *)date format:(NSString *)format {
    return [date wgk_stringWithFormat:format];
}
...
+ (NSUInteger)wgk_daysInMonth:(NSDate *)date {
    return [self wgk_daysInMonth:date month:[date wgk_month]];
}
...
+ (NSString *)wgk_timeInfoWithDateString:(NSString *)dateString {
    ...
    int month = (int)([curDate wgk_month] - [date wgk_month]);
    int year = (int)([curDate wgk_year] - [date wgk_year]);
    int day = (int)([curDate wgk_day] - [date wgk_day]);
...
//        return [NSString stringWithFormat:@"%.0f分钟前", retTime];
    ...
    // 第二个条件是隔年，对于隔年，只能是去年12月与今年1月这种情况
             ...
             || (abs(year) == 1 && [curDate wgk_month] == 1 && [date wgk_month] == 12)) {
        int retDay = 0;
        if (year == 0) { // 同年
            if (month == 0) { // 同月
                retDay = day;
        ...
        if (retDay <= 0) {
            // 获取发布日期中，该月有多少天
            int totalDays = (int)[self wgk_daysInMonth:date month:[date wgk_month]];
            
            // 当前天数 + （发布日期月中的总天数-发布日期月中发布日，即等于距离今天的天数）
            retDay = (int)[curDate wgk_day] + (totalDays - (int)[date wgk_day]);
        }
        ...
        if (abs(year) <= 1) {
            ...
            // 隔年
            int month = (int)[curDate wgk_month];
            int preMonth = (int)[date wgk_month];
